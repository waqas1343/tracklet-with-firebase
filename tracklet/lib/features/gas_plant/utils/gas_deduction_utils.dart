...
class GasDeductionUtils {
  ...
  static Future<bool> deductGasFromTanks({
    ...
    try {
      // Get all tanks
      final QuerySnapshot tanksSnapshot = await FirebaseFirestore.instance
          .collection('tanks')
          .get();
      ...
      if (totalAvailableGas < amountInTons) {
        ...
        CustomFlushbar.showError(
          context,
          message:
              'Not enough gas available. Required: ${amountInTons.toStringAsFixed(2)} tons, Available: ${totalAvailableGas.toStringAsFixed(2)} tons',
        );
        return false;
      }
      ...
      for (final tank in tanks) {
        ...
        if (tank.currentGas > 0) {
          final double deductAmount = remainingAmount < tank.currentGas
              ? remainingAmount
              : tank.currentGas;
          ...
          await FirebaseFirestore.instance
              .collection('tanks')
              .doc(tank.id)
              .update({'currentGas': FieldValue.increment(-deductAmount)});

          remainingAmount -= deductAmount;
        }
      }

      // Get updated total gas after deduction
      final double remainingTotalGas = await getTotalAvailableGasInTons();

      // Show success message with details including remaining gas
      CustomFlushbar.showSuccess(
        context,
        message:
            'Order approved! Deducted ${amountInTons.toStringAsFixed(2)} tons. Remaining gas: ${remainingTotalGas.toStringAsFixed(2)} tons',
      );
